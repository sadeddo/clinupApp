...
const UpdateLogement = () => {
    ...
    const { logementId } = route.params; // Récupérer l'ID du logement à éditer
    ...
    const [fetching, setFetching] = useState(true); // Indicateur de chargement pour les données initiales
    const [error, setError] = useState('');

    // Charger les données du logement à partir de l'API lorsque le composant est monté
    useEffect(() => {
        const fetchLogement = async () => {
            ...
            try {
                ...
                const response = await axios.get(`http://127.0.0.1:8000/api/logements/${logementId}`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                });

                const fetchedLogement = response.data;

                // Mettre à jour les champs avec les données récupérées
                setLogement({
                    nom: fetchedLogement.nom,
                    surface: fetchedLogement.surface,
                    nbrChambre: fetchedLogement.nbrChambre,
                    nbrBain: fetchedLogement.nbrBain,
                    adresse: fetchedLogement.adresse,
                    completAdresse: fetchedLogement.completAdresse,
                    airbnb: fetchedLogement.airbnb,
                    booking: fetchedLogement.booking,
                    acces: fetchedLogement.acces,
                    description: fetchedLogement.description,
                    img: fetchedLogement.img, // Vous pouvez laisser null si aucune image n'existe
                });

                setFetching(false); // Fin du chargement
            } catch (error) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (selectedImage) {
                ...
                formData.append('img', {
                    uri,
                    type: selectedImage.type || 'image/jpeg', // Définir un type par défaut
                    name: selectedImage.fileName || 'image.jpg', // Définir un nom de fichier par défaut
                });
            }
    
            const response = await axios.post(
                `http://127.0.0.1:8000/api/logements/${logementId}/edit`,
                formData,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            Alert.alert('Erreur', 'Une erreur est survenue lors de la mise à jour du logement');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContainer}>
                <Text style={styles.title}>Modifier le logement</Text>
                ...
                <TouchableOpacity style={styles.submitButton} onPress={handleSubmit} disabled={loading}>
                    <Text style={styles.submitButtonText}>{loading ? 'Chargement...' : 'Mettre à jour'}</Text>
                </TouchableOpacity>
            </ScrollView>
            <FooterHote />
        </View>
